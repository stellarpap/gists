...
match (a:DatabaseServer {brand: 'redis'}), (b:Database {name: 'hotels_email'})
create (a)-[:HOSTS]->(b);
...
create (a)-[:CALLED_BY{endpoint:'http://refundservice-v1.production.api.e-travel.gr/refund?format=json',name:'refund_transaction'}]->(b);

match (a:Service{name:'my_bookings'}), (b:Application{name:'panoramix'})
create (a)-[:CALLED_BY{endpoint:'http://mybookings-v1.production.api.e-travel.gr/payments',name:'extra_payment'}]->(b);
